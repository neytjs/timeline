...
class Search extends Component {
  constructor(props) {
    ...
    this.state = {
      ...
      date_start: getGlobal('search').search_arguments.date_start.field,
      date_end: getGlobal('search').search_arguments.date_end.field,
      description: getGlobal('search').search_arguments.description.field,
      rank: getGlobal('search').search_arguments.rank.field,
      searchbydate: getGlobal('search').search_arguments.searchbydate.field,
  ...
  componentWillUnmount() {
    getGlobal('search').search_arguments.description.field = this.description.value;
    getGlobal('enterTracker').tag_insert_tracker = false;
    ...
    if ((this.state.tag === null || this.state.tag.length === 0) && this.state.searchbydate === false && description === "" && (this.state.rank === null || this.state.rank.length === 0)) {
      ...
      if (this.state.searchbydate === true) {
        ...
        if (this.state.date_end && this.state.date_start) {
          ...
          if (getGlobal('search').view_all === true) {
          ...
          getGlobal('search').search_arguments.date_start.searched = start_date_in_ms;
          getGlobal('search').search_arguments.date_end.searched = end_date_in_ms;
      ...
      getGlobal('search').search_arguments.description.field = description;
    }
  }
  ...
  resetSearch() {
    ...
    getGlobal('search').search_arguments.date_start = { field: "", searched: "" };
    getGlobal('search').search_arguments.date_end = { field: "", searched: "" };
    getGlobal('search').search_arguments.description = { field: "", searched: "" };
    getGlobal('search').search_arguments.rank = { field: null, searched: null };
    getGlobal('search').search_arguments.searchbydate = { field: false, searched: false };
    getGlobal('enterTracker').tag_insert_tracker = false;
    getGlobal('enterTracker').component_tracker = "search";
  }
  ...
  handle_date_start_Change(date) {
    this.setState({ date_start: date }, function() {
      getGlobal('search').search_arguments.date_start.field = this.state.date_start;
      getGlobal('enterTracker').tag_insert_tracker = false;
      getGlobal('enterTracker').component_tracker = "search";
    });
  }
  ...
  handle_date_end_Change(date) {
    this.setState({ date_end: date }, function() {
      getGlobal('search').search_arguments.date_end.field = this.state.date_end;
      getGlobal('enterTracker').tag_insert_tracker = false;
      getGlobal('enterTracker').component_tracker = "search";
    });
  }
  ...
  searchByDate(searchby) {
    this.setState({ searchbydate: searchby }, function() {
      getGlobal('search').search_arguments.searchbydate.field = this.state.searchbydate;
        ...
        <div className="theader">
          ...
          <Select
            ...
            onChange={value => this.setState({ rank: value }, function() {
              getGlobal('search').search_arguments.rank.field = value;
              getGlobal('enterTracker').tag_insert_tracker = false;
              getGlobal('enterTracker').component_tracker = "search";
            })}
            options={options}
